...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                .authorizeRequests()
                    .antMatchers( "/res","/css/*","/fonts/icomoon/*","/fonts/icomoon/fonts/*","/fonts/icomoon/demo-files/*","/images/*","/scripts/login/*").permitAll()
...
//                    .antMatchers(HttpMethod.POST, "/xv").hasAuthority("ADMIN")
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.jdbcAuthentication()
                ...
                .authoritiesByUsernameQuery("select u.login, u.role from user_profile u where u.login=?");
    }
}